...
def smooth_max(y, target, derivate=False):
    if derivate == True:
        dif = np.square(y-target)
        #res = np.exp(dif)/sum(np.exp(dif))
...
class NN:
    ...
    def addLayers(self, neurons, activation_fun, weights=None):
        ...
        if weights == None:
            ...
            #weights_hidden = [gen.randn(row, col).astype(np.float32) * math.sqrt(2.0 / row) for row, col in weights_hidden_shapes] 
            weights_hidden = [gen.normal(loc=0., scale = 0.05, size=(row, col)).astype(np.float32) for row, col in weights_hidden_shapes] 
            #bias_hidden = [np.ones((1, n)).astype(np.float32)*0.001 for n in neurons]
            bias_hidden = [gen.normal(loc=0., scale = 0.05, size=(1, n)).astype(np.float32) for n in neurons]
            self.layers = [[w,b] for w, b in list(zip(weights_hidden, bias_hidden))]
            #Wo = gen.randn(neurons[-1], self.N_CLASSES).astype(np.float32) * math.sqrt(2.0 / neurons[-1])
            #bWo = np.ones((1, self.N_CLASSES)).astype(np.float32)*0.001
            Wo = gen.normal(loc=0., scale = 0.05 ,size=(neurons[-1], self.N_CLASSES)).astype(np.float32) * math.sqrt(2.0 / neurons[-1])
            bWo = gen.normal(loc=0., scale = 0.05 ,size=(1, self.N_CLASSES)).astype(np.float32)
            self.layers += [[Wo,bWo]]
        else:
            self.layers=weights
    ...
    def loss(self, X, t):
        ...
        if X.shape[0] > 2**1000:
            ...
            loss = logsumexp(np.square(predictions-t))
        return loss
    ...
    def updateMomentum(self, X, t):
        ...
        for [indexLow,indexHigh] in indexes_minibatchs:
            ...
            deltas = [self.act_fun[-1](y, True) * smooth_max(y, t[indexLow:indexHigh], True)]
            for i in range(self.nHidden):
