...
'''
nn = NN.NN(training=[bin_data, labels], testing=[[0],[0]], lr=1e-2, mu=0.9, output_classes=N_CLASSES, lambd=0, minibatch=64, disableLog=True)
...
#compressioni: 0 --> pruning, 1 --> weightsharing, 2 --> pruning+ws
compression = 2
...
if compression == 2:
    ...
    nn_compress.set_combined_compression(pruning = 50, cluster = [50, 30], weights=w_loaded)
    
loss = nn_compress.train(stop_function=3, num_epochs=20000)
difference = round(time.time() - now, 5)
