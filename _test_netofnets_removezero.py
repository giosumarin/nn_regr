...
for i in [3,7,10]:
    ...
    for spl in concatenated_splits: 
        ...
        for s in range(split):
            ww = np.copy(w)
            nn = NN1.NN(training=[splitted_bin_data[s], splitted_labels[s]], testing=[[0],[0]], lr=1e-2, mu=0.9, output_classes=1, lambd=0, minibatch=64, disableLog=True)
            nn.addLayers(['leakyrelu'], reshape_nozero(ww, first_nonzero))
            nn.set_patience(10)
            now=time.time()
