...
for spl in [2,4]:
    for i in [3,7]:
        ...
        nn = NN1.NN(training=[bin_data, position_labels], testing=[[0],[0]], lr=0.01, mu=0.9, lambd=0, minibatch=64, disableLog=True)
        nn.addLayers(['leakyrelu'])
        loss = nn.train(stop_function=3, num_epochs=20000)
        
        for s in range(split):
            nn = NN1.NN(training=[splitted_bin_data[s], splitted_labels[s]], testing=[[0],[0]], lr=0.02, mu=0.9, lambd=0, minibatch=32, disableLog=True)
            ...
            .format(i, spl, (dim_set/split), max_err[0], round(max_err[0]/(dim_set/split)*100,3), loss, difference, nn.get_memory_usage()))
